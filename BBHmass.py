...
b=b[:-1] #remove last bin edge

rate1=np.zeros(len(b))
rate2=np.zeros(len(b))
rate3=np.zeros(len(b))
for i in range(len(b)):
	if h1_[i]!=0 : rate1[i]=h1[i]/h1_[i] #avoind divsion by zero
	if h2_[i]!=0 : rate2[i]=h2[i]/h2_[i]
	if h3_[i]!=0 : rate3[i]=h3[i]/h3_[i]
	
plt.step(b,rate1, color='blue', where='post', lw=2.)
plt.step(b,rate2,  color='green', where='post', lw=2.)
plt.step(b,rate3,  color='orangered', where='post', lw=2.)
